...
function Copy-FromLocalSource {
    ...
    Copy-Item -LiteralPath (Join-Path $srcDir "librarygame.ps1") -Destination (Join-Path $InstallDir "librarygame.ps1") -Force
    Copy-Item -LiteralPath (Join-Path $srcDir "librarygame.cmd") -Destination (Join-Path $InstallDir "librarygame.cmd") -Force
...
function Copy-FromRemoteSource {
    ...
    Invoke-WebRequest -Uri "$base/src/librarygame.ps1" -OutFile (Join-Path $InstallDir "librarygame.ps1") -UseBasicParsing
    Invoke-WebRequest -Uri "$base/src/librarygame.cmd" -OutFile (Join-Path $InstallDir "librarygame.cmd") -UseBasicParsing
...
if (-not [string]::IsNullOrWhiteSpace($SourceRoot) -and (Test-Path -LiteralPath (Join-Path $SourceRoot "src\librarygame.ps1"))) {
...
Set-Item -Path $AppPathsKey -Value (Join-Path $InstallDir "librarygame.cmd")
